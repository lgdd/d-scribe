...
- **Behavior**: `service-b` returns 500 on downstream lookup for this entity; `service-a` retries 3× ; all retries fail
...
- **Behavior**: frontend JavaScript throws an unhandled exception when rendering the detail page
- **Datadog signals**: RUM Error Tracking issue, error visible in Session Replay, no backend impact (error is client-side only)

#### Slow API Degradation

- **Trigger**: filter or search with a parameter value that maps to a slow backend path (e.g., `category=slow`, `region=slow`, `status=slow`)
- **Human action**: select the filter/category whose label hints at degradation (e.g., *Legacy*, *Archive*, *Overseas*)
- **Behavior**: `api-gateway` injects 3 s latency on this lookup; frontend shows spinner
...
- **Behavior**: entity is published to the queue; worker consumes the message but fails to process it; message is sent to the dead-letter queue (DLQ) after 3 attempts
...
- **Behavior**: `service-a` publishes 9,999 individual messages to the queue; worker falls behind processing them
...
- **Trigger**: Send a request with `Authorization: Bearer EXPIRED_TOKEN_DEMO`
- **Human action**: Log in as user `expired@demo.test` (whose token TTL is set to 1 s in the Keycloak realm export)
...
- **Behavior**: Keycloak logs 10 consecutive failed-login events; account is temporarily locked
...
- **Human action**: Type `RATELIMIT_TEST` in the chat interface
- **Behavior**: `llm-service` simulates a 429 response from the LLM provider; returns a user-facing error
...
- **Human action**: Type `TIMEOUT_TEST` in the chat interface
- **Behavior**: `llm-service` simulates a 60 s timeout from the LLM provider; request times out at 30 s
