...
@Path("/api/mysql/items-with-details")
@Produces(MediaType.APPLICATION_JSON)
public class NplusOneMysqlResource {
    ...
    public List<Map<String, Object>> itemsWithDetails() throws SQLException {
        try (Connection c = ds.getConnection()) {
            ...
            try (ResultSet rs = c.createStatement().executeQuery("SELECT * FROM parents")) {
                while (rs.next()) parents.add(Map.of("id", rs.getObject("id"), "name", rs.getObject("name")));
            }
            for (Map<String, Object> p : parents) {
                try (PreparedStatement ps = c.prepareStatement("SELECT * FROM children WHERE parent_id = ?")) {
                    ...
                    try (ResultSet rs = ps.executeQuery()) { while (rs.next()) children.add(Map.of("id", rs.getObject("id"), "name", rs.getObject("name"))); }
