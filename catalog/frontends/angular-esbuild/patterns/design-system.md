...
> **Angular note:** `src/theme.css` is wired via the `styles` array in `angular.json`, not via a TypeScript import. Do not `import` it in `main.ts`.
...
> **Angular note:** DaisyUI classes work in Angular standalone component templates without any extra imports. Do not add component-level `styles` arrays — all styling is global via `src/theme.css`.
