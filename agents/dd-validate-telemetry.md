...
| `pg_stat_statements` not found | Extension not loaded in PostgreSQL | Add `pg_stat_statements` to `shared_preload_libraries` (e.g. postgres `command: ["postgres", "-c", "shared_preload_libraries=pg_stat_statements"]`); recreate the container with `docker compose up -d --force-recreate postgres`. Ensure `CREATE EXTENSION IF NOT EXISTS pg_stat_statements` runs in the init script in the same database the Agent connects to |
| "pg_stat_statements is not created in database 'postgres'" | Agent connects to default `postgres` but extension/app DB is another database | Set `"dbname": "<actual_database>"` in the postgres Autodiscovery instance (e.g. same as `POSTGRES_DB` or the DB where the extension is created) |
| "explain_statement function might be missing in the datadog schema" | DBM EXPLAIN plans require the datadog schema and helper | Create `datadog` schema, `GRANT USAGE ON SCHEMA datadog TO datadog` (and `public`), and the `datadog.explain_statement(l_query TEXT, OUT explain JSON)` function from the [Postgres DBM setup](https://docs.datadoghq.com/database_monitoring/setup_postgres/selfhosted/). For an already-initialized DB, run a migration script with the same SQL |
| Schema collection / DBM not fully enabled for Postgres | collect_schemas missing or Agent too old | Use the latest Agent image (`gcr.io/datadoghq/agent:latest`) and add `"collect_schemas": {"enabled": true}` to the postgres check instance |
...
| DBM "View trace" / trace link missing for prepared statements | Prepared-statement tracing not enabled for the tracer in use | When using a tracer that supports it (see [Correlate DBM and Traces](https://docs.datadoghq.com/database_monitoring/connect_dbm_and_apm/) supported tracers table), set `DD_DBM_TRACE_PREPARED_STATEMENTS=true` in addition to `DD_DBM_PROPAGATION_MODE=full` (e.g. Java dd-trace-java 1.44+ for Postgres) |
